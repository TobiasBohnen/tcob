...
{
    static auto const nsvg_xformPoint {[](f32* dx, f32* dy, f32 x, f32 y, std::array<f32, 6> const& t) {
        *dx = (x * t[0]) + (y * t[2]) + t[4];
        *dy = (x * t[1]) + (y * t[3]) + t[5];
    }};
    static auto const nsvg_xformVec {[](f32* dx, f32* dy, f32 x, f32 y, std::array<f32, 6> const& t) {
        *dx = (x * t[0]) + (y * t[2]);
        *dy = (x * t[1]) + (y * t[3]);
    }};
