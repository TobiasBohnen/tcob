...
class TCOB_API sdl_system final : public system {
public:
    sdl_system();
    ~sdl_system() override;

    auto controllers() const -> std::unordered_map<i32, std::shared_ptr<controller>> const& override;
    auto first_controller() const -> controller& override;
    auto has_controller() const -> bool override;
