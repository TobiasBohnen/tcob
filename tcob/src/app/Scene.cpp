...
void scene::attach_events()
{
    ...
    _connections.connect(input.KeyUp, [this](auto&& event) { handle_input_event(event, &input::receiver::on_key_up, transform::Identity); });
    _connections.connect(input.KeyDown, [this](auto&& event) { handle_input_event(event, &input::receiver::on_key_down, transform::Identity); });
    _connections.connect(input.TextInput, [this](auto&& event) { handle_input_event(event, &input::receiver::on_text_input, transform::Identity); });
    _connections.connect(input.MouseMotion, [this](auto&& event) { handle_input_event(event, &input::receiver::on_mouse_motion, transform::Identity); });
    _connections.connect(input.MouseButtonDown, [this](auto&& event) { handle_input_event(event, &input::receiver::on_mouse_button_down, transform::Identity); });
    _connections.connect(input.MouseButtonUp, [this](auto&& event) { handle_input_event(event, &input::receiver::on_mouse_button_up, transform::Identity); });
    _connections.connect(input.MouseWheel, [this](auto&& event) { handle_input_event(event, &input::receiver::on_mouse_wheel, transform::Identity); });
    _connections.connect(input.ControllerAxisMotion, [this](auto&& event) { handle_input_event(event, &input::receiver::on_controller_axis_motion, transform::Identity); });
    _connections.connect(input.ControllerButtonDown, [this](auto&& event) { handle_input_event(event, &input::receiver::on_controller_button_down, transform::Identity); });
    _connections.connect(input.ControllerButtonUp, [this](auto&& event) { handle_input_event(event, &input::receiver::on_controller_button_up, transform::Identity); });
}
